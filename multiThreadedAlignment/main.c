	...
	while(i < nWindows){
		...
		for(int i = 1; i <= n; i++){
			for(int j = 1; j <= length; j++){
				...
				int val = max4(0, stdMatrix[i-1][j] -1, stdMatrix[i][j-1] -1, stdMatrix[i-1][j-1] + delta(x[i-1], y[start+j-1]));
				stdMatrix[i][j] = val;
		...
		if(innerMax > maxVal){
			if(my_args->cigar != NULL) free(my_args->cigar);

			int xCord = innerX, yCord = innerY;
			while(stdMatrix[xCord][yCord] > 0){

				if(stdMatrix[xCord][yCord] == (stdMatrix[xCord-1][yCord-1] + delta(x[xCord -1], y[start + yCord - 1]))){
			...
			my_args->location = yCord+start+1;
		}
		i += threads;
	}
	deleteMatrix(std);
...
int main(int argc, char * argv[]){
	...
	fprintf(stderr, "\nCPU seconds (average over %d runs): %lf\n", NITER, cpu_seconds()/NITER);
